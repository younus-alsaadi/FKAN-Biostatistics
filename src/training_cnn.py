...
model_name = "CNN_organl_Data_new"
...
logger.info("Class Indices:", class_indices)  # e.g., {'NORMAL': 0, 'PNEUMONIA': 1}
...
logger.info("Computed Class Weights:", class_weights)
...
plt.close()
logger.info(f"Training plots saved at: {plot_filename}")
...
logger.info(
    classification_report(
        true_labels, binary_predictions, target_names=["NORMAL", "PNEUMONIA"]
    )
)
...
plt.figtext(
    0.98, 0.98, "Batch Size: 4, Epochs:100", fontsize=10, color="black", ha="right"
)
plt.figtext(
    0.98,
    0.96,
...
plt.figtext(
    0.98, 0.92, f"F1 score: {f1 * 100:.2f}%", fontsize=10, color="black", ha="right"
)
...
logger.info(f"Confusion Matrix plot saved at: {conf_matrix_filename}")
plt.close()
